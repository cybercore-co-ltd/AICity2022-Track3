...
class MultiviewVidConvHead_S1K(TSNHead):
    def __init__(self,
                 num_classes,
                 in_channels,
    ...
    def forward(self, x, num_segs, batches=1):
        ...
        x = F.pad(input=x, pad=(0,2,0,0), mode='constant', value=0) #[bs, c, 3, num_clip+2]
        x = self.view_conv(x) # [batch, feature, 1,5]
        x = self.numclip_conv(x)  # [batch, feature, 1,1]
