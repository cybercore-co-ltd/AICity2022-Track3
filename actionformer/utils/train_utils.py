...
def valid_one_epoch(
    ...
    mmaction2_result = {'version': 'VERSION 1.3', 'results': {}}
    ...
    for iter_idx, video_list in enumerate(val_loader, 0):
        ...
        with torch.no_grad():
            ...
            # {"score": 0.3025844693183899, "segment": [30.50666666666667, 56.906666666666666]
            video_id = video_list[0]['video_id']
            anns = []
            for segment, score, label in zip(output[0]['segments'], output[0]['scores'], output[0]['labels']):
                #remove action <3s and >35s
                if float(segment[1]-segment[0])<3: continue
                if float(segment[1]-segment[0])>35: continue
                anns.append({'score': float(score), 'segment': segment.tolist(), 'label': int(label)})
        ...
        mmaction2_result['results'].update({video_id: anns})
        # printing
        if (iter_idx != 0) and iter_idx % (print_freq) == 0:
            # measure elapsed time (sync all kernels)
            torch.cuda.synchronize()
    ...
    if evaluator is not None:
        ...
        if output_file is not None:
            mmcv.dump(mmaction2_result, output_file)
